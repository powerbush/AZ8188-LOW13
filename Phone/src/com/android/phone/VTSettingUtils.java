...
public class VTSettingUtils{
    ...
    public void updateVTEngineerModeValues()
    {
    	...
    	try{
    		emContext = PhoneApp.getInstance().createPackageContext("com.mediatek.engineermode", Context.CONTEXT_INCLUDE_CODE);
    	...
    	/*if(mVTEngineerModeValues.auto_answer)
    	{
    		VTManager.setEM( 2 , 0 , 1);
    	}else
    	{
    		VTManager.setEM( 2 , 0 , 0);
    	}
    	int autoAnswerTime;
    	try{
    		autoAnswerTime = new Integer(mVTEngineerModeValues.auto_answer_time).intValue(); 
    	}catch(Exception e)
    	{
    		autoAnswerTime = 0;
    	}
    	VTManager.setEM( 2 , 1 , autoAnswerTime);*/
