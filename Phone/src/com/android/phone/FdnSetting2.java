...
public class FdnSetting2 extends PreferenceActivity
    ...
    private static final String LOG_TAG = "FdnSetting2";
	...
	mRetryPin2New = retryCount;
        Log.d(LOG_TAG, "getRetryPin2 ="+ retryCount);
        switch (retryCount) {
        case GET_SIM_RETRY_EMPTY:
			
	    Log.d(LOG_TAG, "getRetryPin2,GET_SIM_RETRY_EMPTY");
            return " ";
        case 1:
    ...
    public boolean onPreferenceClick(Preference preference){
        ...
        if (preference == mButtonFDNList) {
        	Log.i(LOG_TAG, "onPreferenceClick mButtonFDNList");
    ...
    private void resetFDNDialog(int strId) {
        if (strId != 0) {
            mButtonEnableFDN.setDialogMessage(getString(strId) + "\n"
        	  ...
        	  Log.d(LOG_TAG, "resetFDNDialog 0");
    ...
    private void toggleFDNEnable(boolean positiveResult) {
		Log.d(LOG_TAG, "toggleFDNEnable"+ positiveResult);
    
        if (!positiveResult) {
        		Log.d(LOG_TAG, "toggleFDNEnable positiveResult is false");
            resetFDNDialog(0);
			mRetryPin2Old = mRetryPin2New;
			Log.d(LOG_TAG, "toggleFDNEnable mRetryPin2Old="+mRetryPin2Old);
            return;
        }
    ...
    private Handler mFDNHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
				...
				Log.d(LOG_TAG, "EVENT_PIN2_ENTRY_COMPLETE,puk2_requested");
                                updateFDNPreference();
                            } else {
			...
			Log.d(LOG_TAG, "EVENT_PIN2_ENTRY_COMPLETE");
						
			mRetryPin2Old = mRetryPin2New;
						
			Log.d(LOG_TAG, "EVENT_PIN2_ENTRY_COMPLETE mRetryPin2Old="+mRetryPin2Old);
                    }
                    break;
            }
        }
    ...
    private void updateEnableFDN() {
	    ...
	    Log.d(LOG_TAG, "updateEnableFDN is FdnEnabled="+R.string.disable_fdn);
            ...
            Log.d(LOG_TAG, "updateEnableFDN is not FdnEnabled="+R.string.enable_fdn);
    ...
    private void updateFDNPreference() {
        ...
        if (getRetryPin2Count() == 0) {
	    mRetryPin2New = 0;
	    Log.d(LOG_TAG, "updateFDNPreference, mRetryPin2New="+mRetryPin2New);
            mButtonEnableFDN.setEnabled(false);
            mButtonFDNList.setEnabled(false);
	    ...
	    Log.d(LOG_TAG, "updateFDNPreference");
            ...
            updateEnableFDN();
			 ...
			 try
             {
		        ...
		        if (getRetryPin2Count() == 0) {
			    mRetryPin2New = 0;
			    Log.d(LOG_TAG, "single updateFDNPreference, mRetryPin2New="+mRetryPin2New);
		            mButtonEnableFDN.setEnabled(false);
		            mButtonFDNList.setEnabled(false);
		            mButtonChangePin2.setTitle(R.string.unblock_pin2);
		            if (getRetryPuk2Count() == 0) {
			    ...
			    Log.d(LOG_TAG, "single updateFDNPreference");
		            mButtonEnableFDN.setEnabled(true);
		            mButtonChangePin2.setEnabled(true);
		            mButtonChangePin2.setTitle(R.string.change_pin2);
		            mButtonChangePin2.setSummary(R.string.sum_fdn_change_pin);
                ...
                {
            	 Log.i(LOG_TAG,"single phone exception ");
                }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        Log.d(LOG_TAG, "onCreate,Sim Id " + mSimId);
	...
	//else{
		...
		Log.d(LOG_TAG, "onCreate,  mRetryPin2Old=" + mRetryPin2Old);
	//}
    ...
    protected void onResume() {
        ...
        Log.d(LOG_TAG, "onResume, mRetryPin2New= "+mRetryPin2New+" mRetryPin2Old="+mRetryPin2Old);
      
        if (mRetryPin2New != mRetryPin2Old){
            mRetryPin2Old = mRetryPin2New;
            Log.d(LOG_TAG, "onResume, fdnDialog= "+fdnDialog);
            if (fdnDialog != null){
                Log.d(LOG_TAG, "onResume, fdnDialog.isShowing()="+fdnDialog.isShowing());
            }
            Log.d(LOG_TAG, "onResume, second mRetryPin2New= "+mRetryPin2New+" mRetryPin2Old="+mRetryPin2Old);

            if (fdnDialog != null && fdnDialog.isShowing()){
               Log.d(LOG_TAG, "onResume, isShowing");
               mButtonEnableFDN.getDialog().dismiss();
            }
            //mButtonEnableFDN.showPinDialog();
        }
    ...
    private void checkPhoneBookStateExit(){
        ...
        Log.d(LOG_TAG,"checkPhoneBookStateExit:isPhoneBookReady="+isPhoneBookReady);
    	if (!isPhoneBookReady) {
    ...
    private boolean checkPhoneBookState(){
        ...
        Log.d(LOG_TAG,"getIccCardGemini("+mSimId+").isPhbReady()="+isPhoneBookReady);
    	if (!isPhoneBookReady) {
    		showTipToast(getString(R.string.error_title),getString(R.string.fdn_phone_book_busy));
    	}
