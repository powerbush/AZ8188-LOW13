...
/* Fion add end */
    ...
    void init(TimeConsumingPreferenceListener listener, boolean skipReading, int simId) {	
        ...
        if (!skipReading) {
            ...
            {
                mSimId = simId;   
                Log.d(LOG_TAG, "init - simId =" + mSimId);
                ...
                if (CallSettings.isMultipleSim())
                {
                    if (mServiceClass == CommandsInterface.SERVICE_CLASS_VIDEO)
                    {
                    	((GeminiPhone)phone).setVtCallForwardingOptionGemini(action,
	                                ...
	                                number,
	                                time,
	                                mHandler.obtainMessage(MyHandler.MESSAGE_SET_CF), mSimId);
                    }else  {
        ...
        private void handleGetCFResponse(Message msg) {
            ...
            if (ar.exception != null) {
                ...
                if (cfInfoArray.length == 0) {
                    ...
                    //Fix ALPS00045601: Network return the serviceClass 580 always
                    if (!foundServiceClass) {
                        if ((reason == 0) && (msg.arg2 == MESSAGE_SET_CF)  && isUserException) {
                            if (isToggled()) {
                                setToggled(false);
                            }
                        }
            ...
            {
                if (isUserException == true || ((tcpListener instanceof GsmUmtsCallForwardOptions) 
        ...
        private void handleSetCFResponse(Message msg) {
            ...
            Log.d(LOG_TAG, "mSimdId: "+mSimId);            
                ...
                if (CallSettings.isMultipleSim())
                {
                        ...
                        {
                            ((GeminiPhone)phone).getVtCallForwardingOptionGemini(reason,
                                    obtainMessage(MESSAGE_GET_CF, reason, MESSAGE_SET_CF, ar.exception), mSimId);
                        }else
                        {
                            ((GeminiPhone)phone).getCallForwardingOptionGemini(reason,
                                    obtainMessage(MESSAGE_GET_CF, reason, MESSAGE_SET_CF, ar.exception), mSimId);
