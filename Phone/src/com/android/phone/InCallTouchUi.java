...
/*Google: private static final int IN_CALL_WIDGET_TRANSITION_TIME = 250; // in ms
...
//MTK end
    ...
    protected void onFinishInflate() {
        ...
        mMuteRawButton = (Button) mInCallControls.findViewById(R.id.muteButton);
        mMuteRawButton.setOnClickListener(this);
        mSpeakerRawButton = (Button) mInCallControls.findViewById(R.id.speakerButton);
...
//MTK begin:
			...
			{
		        log("updateState: OnAnswerAndEndFlag is true");
...
//MTK add below one line:
        ...
        if (showIncomingCallControls) {
        	...
        	{
        		if (DBG) log("***************** mInVTAnswering = "+ isInVTAnswering() );
        		if( isInVTAnswering() )
        		{
        			setInVTAnswering(false);
        			mInCallScreen.internalAnswerVTCall();        		
        		}
        		else
        		{
...
//MTK begin:
            /*
            if(isUpdateIncomingUI && !inCallControlState.manageConferenceVisible)
                mInCallScreen.updateIncomingUi(true);
...
//MTK end
        ...
        /* Added by xingping.zheng start*/
        if(showInCallControls) {
            if(mDialerView.getVisibility() == View.VISIBLE)
    ...
    void updateInCallControls(Phone phone) {
        ...
        */
        ...
        mHoldButton.setVisibility(inCallControlState.canShowSwap ? View.GONE : View.VISIBLE);
        log("canHold = "+inCallControlState.canHold+" onHold = "+inCallControlState.onHold+" canSwap = "+inCallControlState.canSwap);
