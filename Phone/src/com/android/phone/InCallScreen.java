...
//MTK add but comment when merge:private boolean mShowCallLogAfterDisconnect;
	...
	// Added by lei.ye@archermind.com 
...
enum InCallTouchUiType{
    ...
    private ImageButton mVTTakePeerPhoto;
    private ImageButton mVTSwitchCamera;
    private ImageButton mVTHideMe;
    private ImageButton mVTMute;
    private ImageButton mVTSpeaker;
    ...
    private ImageView mVTElapsedTimeIcon;
    private ImageView mVTPhoneNumberIcon;
...
/* Fion add end */										
                ...
                case DELAY_AUTO_ANSWER:
                	...
                	{
                		if( VTCallUtils.VTScreenMode.VT_SCREEN_RINGING == getVTScreenMode() )
                		{
                			break;
                		}
                	}
...
/*MTK add:
    ...
    /* package */ void updateKeyguardPolicy(boolean dismissKeyguard) {
        ...
        if (dismissKeyguard) {
            log("updateKeyguardPolicy: set window flag ");
            getWindow().addFlags(WindowManager.LayoutParams.FLAG_DISMISS_KEYGUARD);
        } else {
            log("updateKeyguardPolicy: clear window flag ");
    ...
    private void registerForPhoneStates() {
        if (!mRegisteredForPhoneStates) {
            ...
            if( FeatureOption.MTK_VT3G324M_SUPPORT == true )
            {
            	...
            	/*if (FeatureOption.MTK_GEMINI_SUPPORT)
            	{
            		if (DBG) log("- mCMGemini.registerForStartAcceptVtGemini(mHandler, VT_PRE_ANSWER, null, Phone.GEMINI_SIM_1) !");
            		mCMGemini.registerForStartAcceptVtGemini(mHandler, VT_PRE_ANSWER, null, Phone.GEMINI_SIM_1); 
            	}else
            	{
            		if (DBG) log("- mCM.registerForStartAcceptVt(mHandler, VT_PRE_ANSWER, null) !");
            		mCM.registerForStartAcceptVt(mHandler, VT_PRE_ANSWER, null);  
            	}*/
            }
            /*Added by lianyu.zhang end*/
    ...
    private void unregisterForPhoneStates() {
            ...
            if( FeatureOption.MTK_VT3G324M_SUPPORT == true )
            {
            	...
            	/*if (FeatureOption.MTK_GEMINI_SUPPORT)
            	{
            		if (DBG) log("- mCMGemini.unregisterForStartAcceptVtGemini(mHandler, Phone.GEMINI_SIM_1) ! ");
            		mCMGemini.unregisterForStartAcceptVtGemini(mHandler, Phone.GEMINI_SIM_1); 
            	}else
            	{
            		if (DBG) log("- mCM.unregisterForStartAcceptVt(mHandler) ! ");
            		mCM.unregisterForStartAcceptVt(mHandler);  
            	}*/
            }
            /*Added by lianyu.zhang end*/
            ...
            if( FeatureOption.MTK_VT3G324M_SUPPORT == true )
            {
            	if( mPhone.getState() == Phone.State.RINGING )
            	{
            		if( getVTScreenMode() == VTCallUtils.VTScreenMode.VT_SCREEN_OPEN )
            		{
            			...
            			{
            				setVTScreenMode(VTCallUtils.VTScreenMode.VT_SCREEN_RINGING);
            			}
            			else
            			{
            				setVTScreenMode( VTCallUtils.VTScreenMode.VT_SCREEN_HIDING );
            			}
            		}
            		else
            		{
    ...
    private void initInCallScreen() {
        ...
        mCallStatus = (CallStatus) findViewById(R.id.call_status);
...
//MTK end
    ...
    private void onPhoneStateChanged(AsyncResult r) {
        ...
        log("onPhoneStateChanged: phoneIsInUse: " + phoneIsInUse());
        log("onPhoneStateChanged: mInCallScreenMode: " + mInCallScreenMode);
        if(null != mForegroundCall) log("onPhoneStateChanged: mForegroundCall.getState() : " + mForegroundCall.getState());
        if(null != mBackgroundCall) log("onPhoneStateChanged: mBackgroundCall.getState() : " + mBackgroundCall.getState());        
        if(null != mRingingCall) log("onPhoneStateChanged: mRingingCall.getState() : " + mRingingCall.getState());
           ...
           && Call.State.HOLDING != mBackgroundCall.getState()))
        {
            ...
            {
                log("onPhoneStateChanged: the call state not changed, no update");
                return;
            }
        }
...
/* Fion add start */     
    private void onDisconnect(AsyncResult r , int msg) {
        ...
        if (cause == Connection.DisconnectCause.CALL_BARRED) {
        	...
        	Log.d("InCallScreen", "Check VT call dropback");
        	if (cause == Connection.DisconnectCause.INCOMPATIBLE_DESTINATION) {
                ...
                showReCallDialog(R.string.callFailed_dsac_vt_bear_not_authorized);
                return;
            	...
            	Log.d("InCallScreen",
					...
					Log.d("InCallScreen", "VT call dropback nCSNetType = "
							+ nCSNetType);
...
/* MTK end */
            ...
            log("- onDisconnect: currentlyIdle:"+currentlyIdle+" ; mForegroundCall.getState():"+mForegroundCall.getState());
...
/* MTK begin:
    ...
    private InCallInitStatus placeCall(Intent intent) {
        ...
        // Google: InCallInitStatus okToCallStatus = checkIfOkToInitiateOutgoingCall(mCM.getServiceState());
/*MTK   InCallInitStatus okToCallStatus; 
        int simId;
	    ...
	    okToCallStatus = checkIfOkToInitiateOutgoingCallGemini(simId);
        } else {
               okToCallStatus = checkIfOkToInitiateOutgoingCall();
        } */
        /* need to check if one of sim has call ready */
...
//MTK add below line:
        ...
        if (number.length() > MAX_PIN_LENGTH) {
            number = number.substring(0, MAX_PIN_LENGTH);
        }
    	...
    	if (isVTCall)
    	{
    		...
    		else
    		{
    			...
    			else
    			{
    	    		...
    	    		if (VDBG) log("- LowSurface: "+ this.mVTLowVideo.toString());
    	    		if (VDBG) log("- HighSurface: "+ this.mVTHighVideo.toString());
...
//MTK add below line:
            ...
            case PhoneUtils.CALL_STATUS_FAILED:
                ...
                if( FeatureOption.MTK_VT3G324M_SUPPORT == true )
                {
                	if( VTCallUtils.VTScreenMode.VT_SCREEN_OPEN == getVTScreenMode() )
                	{
                		if (VDBG) log("- VT : CALL_STATUS_FAILED ! ");
...
//MTK remove:     .setTitle(R.string.no_vm_number)
    ...
    private void startRecord() {
		PhoneApp.getInstance().startService(mRecorderServiceIntent);
    ...
    private void stopRecord() {
		PhoneApp.getInstance().stopService(mRecorderServiceIntent);
    }
    ...
    */
    ...
    public void onClick(View view) {
        ...
        switch (id) {
			// Added by lei.ye@archermind.com
	    	/*case R.id.record: mtk add
				if (mExpandedMenuView != null) {	
					if (!phoneRecorder.ismFlagRecord()) {
						startRecord();
					} else {
						stopRecord();
				...
				break;*/
			// Added bylei.ye@archermind.com end
            case R.id.menuAnswerAndHold:
                if (VDBG) log("onClick: AnswerAndHold...");
                /*Added by lianyu.zhang start*/
            	if( FeatureOption.MTK_VT3G324M_SUPPORT == true )
            ...
            */
            ...
            case R.id.VTLowVideo:
            	...
            	break;
            case R.id.VTTakePeerPhoto:
            	if(VDBG)log("onClick: VTTakePeerPhoto...");
            	onVTTakePeerPhotoClick();
            	break;
            	
            case R.id.VTSwitchCamera:
            	if(VDBG)log("onClick: VTSwitchCamera...");
            	onVTSwitchCameraClick();
            	break;
            	
            case R.id.VTHideMe:
            	if(VDBG)log("onClick: VTHideMe...");
            	onVTHideMeClick();
            	break;
            	
            case R.id.VTMute:
            	if(VDBG)log("onClick: VTMute...");
            	onMuteClick();
            	break;
...
//        }
    ...
    private void onAddCallClick() {
        if(FeatureOption.MTK_GEMINI_SUPPORT) {
            log("onAddCallClick");
            Call call;
            boolean phoneIsIdle = false;
            int currentSlot = -1;
            long currentSim = Settings.System.DEFAULT_SIM_NOT_SET;
            ...
            if(call != null) {
                ...
                if(!phoneIsIdle) {
                    if(call.getPhone().getPhoneType() == Phone.PHONE_TYPE_SIP)
                        currentSim = Settings.System.VOICE_CALL_SIM_SETTING_INTERNET;
                    else {
                        GeminiPhone phone = (GeminiPhone)mPhone;
                        if(phone.getStateGemini(Phone.GEMINI_SIM_2) != Phone.State.IDLE) {
                            currentSlot = Phone.GEMINI_SIM_2;
                        } else if(phone.getStateGemini(Phone.GEMINI_SIM_1) != Phone.State.IDLE) {
                            currentSlot = Phone.GEMINI_SIM_1;
                        }
                        SIMInfo simInfo = SIMInfo.getSIMInfoBySlot(this, currentSlot);
                        if(simInfo != null)
                            currentSim = simInfo.mSimId;
                    }
                    log("onAddCallClick, currentSim = "+currentSim);
                }
            }
            log("onAddCallClick, defaultSim = "+defaultSim+" phoneIsIdle = "+phoneIsIdle);
            if(!phoneIsIdle && currentSim != defaultSim) {  
                if(mSimCollisionDialog != null && mSimCollisionDialog.isShowing()) {
                    log("onAddCallClick, mSimCollisionDialog is showing, bail out...");
                    return;
                }
                AlertDialog.Builder builder = new AlertDialog.Builder(this);
                mSimCollisionDialog = builder.setIcon(android.R.drawable.ic_dialog_alert)
                                             .setTitle(R.string.incall_sim_collision_dialog_title)
                                             .setMessage(R.string.incall_sim_collision_dialog_message)
                                             .setPositiveButton(R.string.pause_prompt_yes, mDialogClickListener)
                                             .setNegativeButton(R.string.pause_prompt_no, null)
                                             .create();
                mSimCollisionDialog.show();
            } else
                PhoneUtils.startNewCall(mCM);
        } else {
            PhoneUtils.startNewCall(mCM);
    ...
    /* package */ void handleOnscreenButtonClick(int id) {
        ...
        switch (id) {
            ...
            case R.id.rejectButton:
            	...
            	if( FeatureOption.MTK_VT3G324M_SUPPORT == true )
            	{
            		if( getVTScreenMode() == VTCallUtils.VTScreenMode.VT_SCREEN_RINGING &&
...
//MTK begin:
    ...
    private void makeVoiceCallEx(Context context, String number, int slot) {
        ...
        if(FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            log("makeVoiceCallEx, intent = "+intent);
        } else {
	...
	private void showReCallDialog(int resid) {
		...
		clickListener = new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				if (DBG)
					log("showReCallDialog... , on click, which=" + which);
				/* Fion add start */
				if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
					delayedCleanupAfterDisconnect(DELAYED_CLEANUP_AFTER_DISCONNECT2);
				}
				...
				if (-1 == which) {
					String number = null;
					String action = intent.getAction();
					log("showReCallDialog... intent action is " + action);
					...
					log("showReCallDialog...number is " + number);

/*					Intent newIntent = new Intent(Intent.ACTION_CALL_PRIVILEGED,
							Uri.fromParts("tel", number, null));
					newIntent.putExtra(
									com.android.internal.telephony.Phone.GEMINI_SIM_ID_KEY,
									0);
					newIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
							| Intent.FLAG_ACTIVITY_CLEAR_TOP);
					startActivity(newIntent);*/
					makeVoiceCallEx(getApplicationContext(), number, intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, -1));
				}
			}
		};
		cancelListener = new OnCancelListener() {
		...
		mCanDismissDialog = new AlertDialog.Builder(this).setMessage(msg)
				.setNegativeButton(android.R.string.cancel, clickListener)
				.setPositiveButton(R.string.ok, clickListener)
				.setOnCancelListener(cancelListener).create();
		mCanDismissDialog.getWindow().addFlags(
				WindowManager.LayoutParams.FLAG_DIM_BEHIND);
    ...
    public boolean isPhoneStateRestricted(int simId)
    {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {            
            ...
            return ((serviceStateGemini == ServiceState.STATE_EMERGENCY_ONLY) ||
                    (serviceStateGemini == ServiceState.STATE_OUT_OF_SERVICE && mLastDisconnectCause != Connection.DisconnectCause.LOCAL
                                                                             && mLastDisconnectCause != Connection.DisconnectCause.NORMAL) ||
                (PhoneApp.getInstance().getKeyguardManager().inKeyguardRestrictedInputMode()));
        } else {
            int serviceState = mCM.getServiceState();
            return ((serviceState == ServiceState.STATE_EMERGENCY_ONLY) ||
                (serviceState == ServiceState.STATE_OUT_OF_SERVICE && mLastDisconnectCause != Connection.DisconnectCause.LOCAL
                                                                   && mLastDisconnectCause != Connection.DisconnectCause.NORMAL) ||
                (PhoneApp.getInstance().getKeyguardManager().inKeyguardRestrictedInputMode()));
        }
    }
...
//MTK add below one line:
        int title_frame_backgroundResId = R.drawable.bg_call_sim;
...
//MTK end
    ...
    public boolean onCreateOptionsMenu(Menu menu){
    	...
    	menu.add(0, R.id.vtmenuitemBluetoothon,0,R.string.menu_bluetooth_on).setIcon(R.drawable.vt_incall_menu_bluetoothon);
    	menu.add(0, R.id.vtmenuitemSwapvideos, 0, R.string.vt_swap_video).setIcon(R.drawable.vt_incall_menu_swapvideo);
    	menu.add(0, R.id.vtmenuitemDialpad, 0, R.string.onscreenShowDialpadText).setIcon(R.drawable.vt_incall_menu_dialpad);
    	menu.add(0, R.id.vtmenuitemVideosettings, 0, R.string.vt_settings).setIcon(R.drawable.vt_incall_menu_videosetting);
    	
        /* Added by xingping.zheng start */
        menu.add(0, SEND_MESSAGE, 0, R.string.send_message_after_reject).setIcon(R.drawable.incall_sendsms_menu);
        menu.add(0, OPTION_MENU_ITEM_MUTE, 0, R.string.menu_mute);
        menu.add(0, OPTION_MENU_ITEM_BLUETOOTH, 0, R.string.menu_bluetooth);
        menu.add(0, OPTION_MENU_ITEM_START_RECORDING, 0, R.string.start_record);
        menu.add(0, OPTION_MENU_ITEM_HANGUP_ALL, 0, R.string.menu_disconnect_all);
        menu.add(0, OPTION_MENU_ITEM_DISCONNECT_HELD, 0, R.string.menu_disconnect_hold);
        menu.add(0, OPTION_MENU_ITEM_ECT, 0, R.string.menu_ect);
        menu.add(0, OPTION_MENU_ITEM_HANGUP_ACTIVE_ANSWER_WAITING, 0, R.string.menu_answer_end_active);
        /* Added by xingping.zheng end   */
        return true;
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if( getVTScreenMode() != VTCallUtils.VTScreenMode.VT_SCREEN_OPEN ) {
            ...
            menu.findItem(R.id.vtmenuitemBluetoothon).setVisible(false);
            menu.findItem(R.id.vtmenuitemSwapvideos).setVisible(false);
            menu.findItem(OPTION_MENU_ITEM_DISCONNECT_HELD).setVisible(mCM.hasActiveBgCall());
            ...
            if(!mInCallTouchUi.isIncomingControlShown()) {
                ...
                menu.findItem(OPTION_MENU_ITEM_HANGUP_ALL).setVisible(canHangupAll());
                //mtk71029 update for alps00039248, check the bluetooth state.
                ...
                MenuItem bluetoothItem = menu.findItem(OPTION_MENU_ITEM_BLUETOOTH);
                bluetoothItem.setVisible(true);
                if(mbluetoothEnabled){
                	bluetoothItem.setEnabled(true);
                	if(mbluetoothIndicatorOn){
                		bluetoothItem.setTitle(getResources().getString(R.string.menu_bluetooth_off));
                	}else{
                		bluetoothItem.setTitle(getResources().getString(R.string.menu_bluetooth_on));
                	}
                }else{
                	bluetoothItem.setEnabled(false);
                  bluetoothItem.setTitle(getResources().getString(R.string.menu_bluetooth));
                }
                //menu.findItem(OPTION_MENU_ITEM_BLUETOOTH).setVisible(true);
                //mtk71029 update end.                

                // update start/stop record menu item
                MenuItem recordItem = menu.findItem(OPTION_MENU_ITEM_START_RECORDING);
                PhoneRecorder phoneRecorder = PhoneRecorder.getInstance(this.getApplicationContext());
                ...
                if(!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                    ...
                    /*Call fgCall = mCM.getActiveFgCall();
                    Call bgCall = mCM.getFirstActiveBgCall();
                    if(fgCall.getState() == Call.State.IDLE && bgCall.getState() == Call.State.HOLDING ||
                       fgCall.getState() == Call.State.DIALING || 
                       fgCall.getPhone().getPhoneType() == Phone.PHONE_TYPE_SIP ) {
                        recordItem.setEnabled(false);
                    } else {
                        recordItem.setEnabled(true);
                    }*/
                	log("fgCall.getState():"+ mCM.getActiveFgCall().getState());
                	log("mCM.getFgPhone().getPhoneType():"+ mCM.getFgPhone().getPhoneType());
                	Call fgCall = mCM.getActiveFgCall();
                	if(fgCall.getState() != Call.State.ACTIVE || fgCall.getState() == Call.State.ACTIVE && mCM.getFgPhone().getPhoneType() == Phone.PHONE_TYPE_SIP) {
                ...
                //ECT Menu
                if (mCM.hasActiveFgCall() && mCM.hasActiveBgCall()) {
            	    int FgphoneType = mCM.getActiveFgCall().getPhone().getPhoneType();
            	    int BgphoneType = mCM.getFirstActiveBgCall().getPhone().getPhoneType();
            	    if (FgphoneType != Phone.PHONE_TYPE_SIP && BgphoneType != Phone.PHONE_TYPE_SIP) {
                ...
                /* there is no "send message" menu item when the incoming call is sip call */
            	int phoneType = mCM.getFirstActiveRingingCall().getPhone().getPhoneType();
                menu.findItem(SEND_MESSAGE).setVisible(phoneType != Phone.PHONE_TYPE_SIP);
                ...
                if (mCM.hasActiveFgCall() && mCM.hasActiveRingingCall()) {
                	menu.findItem(OPTION_MENU_ITEM_HANGUP_ACTIVE_ANSWER_WAITING).setVisible(true);
                } else {
                	menu.findItem(OPTION_MENU_ITEM_HANGUP_ACTIVE_ANSWER_WAITING).setVisible(false);
                }
                
                menu.findItem(OPTION_MENU_ITEM_BLUETOOTH).setVisible(false);
                menu.findItem(OPTION_MENU_ITEM_START_RECORDING).setVisible(false);
            ...
            menu.findItem(R.id.vtmenuitemBluetoothon).setVisible(true);
	    	...
	    	menu.findItem(OPTION_MENU_ITEM_ECT).setVisible(false);
	    	menu.findItem(OPTION_MENU_ITEM_HANGUP_ACTIVE_ANSWER_WAITING).setVisible(false);
            MenuItem vtmenuitemBluetoothon= menu.findItem(R.id.vtmenuitemBluetoothon);
            boolean mbluetoothEnabled = getUpdatedInCallControlState().bluetoothEnabled;
            boolean mbluetoothIndicatorOn = getUpdatedInCallControlState().bluetoothIndicatorOn;
            
            if(DBG) log("--onPrepareOptionsMenu! bluetoothEnabled = "+mbluetoothEnabled);
            if(DBG) log("--onPrepareOptionsMenu! bluetoothIndicatorOn = "+mbluetoothIndicatorOn);
            ...
            {
                if(mbluetoothIndicatorOn)
                    vtmenuitemBluetoothon.setTitle(getResources().getString(R.string.menu_bluetooth_off));
                else 
                    vtmenuitemBluetoothon.setTitle(getResources().getString(R.string.menu_bluetooth_on));
            }
    
            menu.findItem(R.id.vtmenuitemDialpad).setEnabled(getUpdatedInCallControlState().dialpadEnabled);
    		menu.findItem(R.id.vtmenuitemVideosettings).setEnabled(VTInCallScreenFlags.getInstance().mVTVideoConnected);
    		menu.findItem(R.id.vtmenuitemSwapvideos).setEnabled(VTInCallScreenFlags.getInstance().mVTVideoConnected);
    ...
    public boolean onOptionsItemSelected(MenuItem item){
    	...
    	case R.id.vtmenuitemBluetoothon:
    		if(DBG) log("onOptionsItemSelected: Bluetooth");
    		onBluetoothClick();
    		return true;
    	...
    	case R.id.vtmenuitemVideosettings:
        	...
        	//Intent intent = new Intent(this, VTInCallVideoSetting.class);
...
//			PhoneUtils.hangupRingingCall(mPhone);
        	internalHangupRingingCall();
//        	if (FeatureOption.MTK_GEMINI_SUPPORT){
//        		PhoneUtils.hangupRingingCall(mCMGemini.getFirstActiveRingingCall());        		
//        	}else 
//        		PhoneUtils.hangupRingingCall(mCM.getFirstActiveRingingCall());
    ...
    {
    	if(DBG) log("initVTInCallCanvas()...");
    	
    	mVTPowerManager = (PowerManager) getSystemService(Context.POWER_SERVICE);
        mVTWakeLock = mVTPowerManager.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "VTWakeLock");
    	...
    	((RelativeLayout)findViewById(R.id.RelativeLayout01)).setBackgroundResource(R.drawable.vt_incall_peer_bkg);
    	((RelativeLayout)findViewById(R.id.RelativeLayout11)).setBackgroundResource(R.drawable.vt_incall_text);
    	((RelativeLayout)findViewById(R.id.RelativeLayout10)).setBackgroundResource(R.drawable.vt_incall_bkg);
    	((RelativeLayout)findViewById(R.id.RelativeLayout02)).setBackgroundResource(R.drawable.vt_incall_local_bkg);
    	
    	mVTHighUp = (ImageButton)findViewById(R.id.VTHighUp);
    	mVTHighUp.setBackgroundResource(R.drawable.vt_incall_button_background2);
    	mVTHighUp.setOnClickListener(this);
    	mVTHighUp.setVisibility(View.GONE);
    	
    	mVTHighDown = (ImageButton)findViewById(R.id.VTHighDown);
    	mVTHighDown.setBackgroundResource(R.drawable.vt_incall_button_background2);
    	...
    	mVTLowDown.setBackgroundResource(R.drawable.vt_incall_button_background2);
    	...
    	mVTTakePeerPhoto = (ImageButton) findViewById(R.id.VTTakePeerPhoto);
    	mVTTakePeerPhoto.setFocusable(true);
    	mVTTakePeerPhoto.setFocusableInTouchMode(false);
    	mVTTakePeerPhoto.setBackgroundResource(R.drawable.vt_incall_button_background);
    	mVTTakePeerPhoto.setImageResource(R.drawable.vt_incall_button_takepeerphoto);
        
    	mVTSwitchCamera = (ImageButton) findViewById(R.id.VTSwitchCamera);
    	mVTSwitchCamera.setFocusable(true);
    	mVTSwitchCamera.setFocusableInTouchMode(false);
    	mVTSwitchCamera.setBackgroundResource(R.drawable.vt_incall_button_background);
    	mVTSwitchCamera.setImageResource(R.drawable.vt_incall_button_switchcamera);
    	
        mVTHideMe = (ImageButton) findViewById(R.id.VTHideMe);
        mVTHideMe.setFocusable(true);
        mVTHideMe.setFocusableInTouchMode(false);
        mVTHideMe.setBackgroundResource(R.drawable.vt_incall_button_background);
        mVTHideMe.setImageResource(R.drawable.vt_incall_button_hideme);
        
        mVTMute = (ImageButton) findViewById(R.id.VTMute);
        mVTMute.setFocusable(true);
        mVTMute.setFocusableInTouchMode(false);
        mVTMute.setBackgroundResource(R.drawable.vt_incall_button_background);
        mVTMute.setImageResource(R.drawable.vt_incall_button_mute);
        
        mVTSpeaker = (ImageButton) findViewById(R.id.VTSpeaker);
        mVTSpeaker.setFocusable(true);
        mVTSpeaker.setFocusableInTouchMode(false);
        mVTSpeaker.setBackgroundResource(R.drawable.vt_incall_button_background);
        mVTSpeaker.setImageResource(R.drawable.vt_incall_button_speaker);
        
        mVTHangUp = (ImageButton) findViewById(R.id.VTHangUp);
        mVTHangUp.setFocusable(true);
        mVTHangUp.setFocusableInTouchMode(false);
        mVTHangUp.setBackgroundResource(R.drawable.vt_incall_button_background);
        mVTHangUp.setImageResource(R.drawable.vt_incall_button_hangup);
       
        mVTTakePeerPhoto.setOnClickListener(this);
        mVTSwitchCamera.setOnClickListener(this);
        mVTHideMe.setOnClickListener(this);
        ...
        mVTElapsedTime = (TextView) findViewById(R.id.vtElapsedTime);        
        mVTElapsedTime.setFocusable(false);
        mVTElapsedTime.setFocusableInTouchMode(false);
        ...
        mVTElapsedTimeIcon = (ImageView) findViewById(R.id.vtElapsedTimeIcon);
        mVTElapsedTimeIcon.setFocusable(false);
        mVTElapsedTimeIcon.setFocusableInTouchMode(false);
        mVTElapsedTimeIcon.setImageResource(R.drawable.vt_incall_image1);
        
        mVTPhoneNumberIcon = (ImageView) findViewById(R.id.vtPhoneNumberIcon);
        mVTPhoneNumberIcon.setFocusable(false);
        mVTPhoneNumberIcon.setFocusableInTouchMode(false);
        
        mVTElapsedTime.setClickable(false);        
        mVTPhoneNumber.setClickable(false); 
        mVTElapsedTimeIcon.setClickable(false);
        mVTPhoneNumberIcon.setClickable(false);
        
        //set focus start
        mVTPhoneNumber.setNextFocusDownId(R.id.VTTakePeerPhoto);
        ...
        mVTTakePeerPhoto.setNextFocusDownId(R.id.VTSwitchCamera);
        mVTTakePeerPhoto.setNextFocusLeftId(R.id.VTTakePeerPhoto);
        mVTTakePeerPhoto.setNextFocusRightId(R.id.VTMute);
        mVTTakePeerPhoto.setNextFocusUpId(R.id.vtPhoneNumber);
        
        mVTSwitchCamera.setNextFocusDownId(R.id.VTHideMe);
        mVTSwitchCamera.setNextFocusLeftId(R.id.VTSwitchCamera);
        mVTSwitchCamera.setNextFocusRightId(R.id.VTSpeaker);
        mVTSwitchCamera.setNextFocusUpId(R.id.VTTakePeerPhoto);
        
        mVTHideMe.setNextFocusDownId(R.id.VTHideMe);
        mVTHideMe.setNextFocusLeftId(R.id.VTHideMe);
        mVTHideMe.setNextFocusRightId(R.id.VTHangUp);
        mVTHideMe.setNextFocusUpId(R.id.VTSwitchCamera);
        
        mVTMute.setNextFocusDownId(R.id.VTSpeaker);
        mVTMute.setNextFocusLeftId(R.id.VTTakePeerPhoto);
        ...
        mVTSpeaker.setNextFocusLeftId(R.id.VTSwitchCamera);
        ...
        mVTHangUp.setNextFocusLeftId(R.id.VTHideMe);
    ...
    {
    	if(DBG)log("closeVTInCallCanvas!");
    	mVTInCallCanvas.setClickable(false);
    	mVTInCallCanvas.setVisibility(View.GONE);
    ...
    public void updateVTScreen( VTCallUtils.VTScreenMode mode )
    {
    	...
    	if(mode != VTCallUtils.VTScreenMode.VT_SCREEN_OPEN || isDialerOpened()){
    		closeVTInCallCanvas();
    		return;
    	}
    	...
    	if(!PhoneApp.getInstance().isVTIdle())
    	{
    		if(VTInCallScreenFlags.getInstance().mVTIsMT)
    			mVTPhoneNumberIcon.setImageResource(R.drawable.vt_incall_image2_in);
    		else
    			mVTPhoneNumberIcon.setImageResource(R.drawable.vt_incall_image2_out);
    	}
    	
		Call.State mState = mForegroundCall.getState();
		if (mState == Call.State.ACTIVE || mState == Call.State.DISCONNECTED
				|| mState == Call.State.DISCONNECTING) {
			updateVTElapsedTime(View.VISIBLE);
		} else {
			updateVTElapsedTime(View.GONE);
			updateVTElapsedTime("00:00");
		}
		...
		if (VTInCallScreenFlags.getInstance().mVTInEndingCall) {
			...
			mVTTakePeerPhoto.setEnabled(false);
			mVTSwitchCamera.setEnabled(false);
			mVTHideMe.setEnabled(false);
			...
			boolean enabled = VTInCallScreenFlags.getInstance().mVTVideoConnected;
			mVTTakePeerPhoto.setEnabled(enabled);

			// enabled = VTInCallScreenFlags.getInstance().mVTVideoReady;
			enabled = true;
			mVTSwitchCamera.setEnabled(enabled
					&& VTSettingUtils.getInstance().mEnableBackCamera
					&& (!VTInCallScreenFlags.getInstance().mVTHideMeNow));
			mVTHideMe.setEnabled(enabled);
		}

        if(DBG)log("updateVTScreen : VTInCallScreenFlags.getInstance().mVTHideMeNow - "+VTInCallScreenFlags.getInstance().mVTHideMeNow);
        if(DBG)log("updateVTScreen : VTSettingUtils.getInstance().mEnableBackCamera - "+VTSettingUtils.getInstance().mEnableBackCamera);
        if(DBG)log("updateVTScreen : mVTSwitchCamera - "+mVTSwitchCamera.isEnabled());
        if(DBG)log("updateVTScreen : mVTHideMe - "+mVTHideMe.isEnabled());
    ...
    {
    	if(DBG)log("updateVTInCallButtonsSrc()...");
    	
    	if(DBG)log("updateVTInCallButtonsSrc() : update mVTSwitchCamera 's src !");
    	if( VTInCallScreenFlags.getInstance().mVTFrontCameraNow )
    		mVTSwitchCamera.setImageResource(R.drawable.vt_incall_button_switchcamera);
    	else
    		mVTSwitchCamera.setImageResource(R.drawable.vt_incall_button_switchcamera_2);
    	
    	if(DBG)log("updateVTInCallButtonsSrc() : update mVTHideMe 's src !");
    	if( !VTInCallScreenFlags.getInstance().mVTHideMeNow )
    		mVTHideMe.setImageResource(R.drawable.vt_incall_button_hideme);
    	else
    		mVTHideMe.setImageResource(R.drawable.vt_incall_button_hideme_2);
    	
    	if(DBG)log("updateVTInCallButtonsSrc() : update mVTMute 's src !");
    	if( PhoneUtils.getMute() )
    		mVTMute.setImageResource(R.drawable.vt_incall_button_mute);
    	else
    		mVTMute.setImageResource(R.drawable.vt_incall_button_mute_2);
    	
    	if(DBG)log("updateVTInCallButtonsSrc() : update mVTSpeaker 's src !");
    	if( !PhoneUtils.isSpeakerOn(this) )
    		mVTSpeaker.setImageResource(R.drawable.vt_incall_button_speaker);
    	else
    		mVTSpeaker.setImageResource(R.drawable.vt_incall_button_speaker_2);
    }
    
    public void setVTScreenMode( VTCallUtils.VTScreenMode mode )
    {
    	if(DBG)log("setVTScreenMode : "+ mode);
    	
    	if( VTCallUtils.VTScreenMode.VT_SCREEN_OPEN != getVTScreenMode()  &&
    			VTCallUtils.VTScreenMode.VT_SCREEN_OPEN == mode )
    	{
    		if(DBG)log("setVTScreenMode : mVTWakeLock.acquire() ");
    		try{
    		...
    		{
    			if(DBG)log("setVTScreenMode : mVTWakeLock.acquire() unsuccessfully , exception !");
    		}
    	}
    	...
    	if( VTCallUtils.VTScreenMode.VT_SCREEN_OPEN == getVTScreenMode()  &&
    			VTCallUtils.VTScreenMode.VT_SCREEN_OPEN != mode )
    	{
    		if(DBG)log("setVTScreenMode : mVTWakeLock.release() ");
    		try{
    			if(mVTWakeLock.isHeld())
    				mVTWakeLock.release();
    		}
    		...
    		{
    			if(DBG)log("setVTScreenMode : mVTWakeLock.release() unsuccessfully , exception !");
    		}
    	}
    ...
    /*package*/void updateVTElapsedTime(int visibility)
    {
    	if(getVTScreenMode() != VTCallUtils.VTScreenMode.VT_SCREEN_OPEN)
    		return;
    	
    	mVTElapsedTimeIcon.setVisibility(visibility);
    	mVTElapsedTime.setVisibility(visibility);
    }
    
    /*package*/void updateVTElapsedTime(CharSequence text)	
    {
    	Log.d("VT/TIME", " " + text);
		
    	mVTElapsedTime.setText(text);
    }
    ...
    {	
    	...
    	if(PhoneApp.getInstance().isVTActive())
    	{
    		...
    		if (VDBG) log("- set/reset VTManager close ! ");
    		VTManager.getInstance().setVTClose();
    		if (VDBG) log("- finish set/reset VTManager close ! ");
    ...
    {
    	if (DBG) log("onVTTakePeerPhotoClick()...");
    	...
    	{
    		Toast.makeText(this, getResources().getString(R.string.vt_sd_null), Toast.LENGTH_SHORT).show();
    		return;
    	}else
    	{
            ...
            {
            	Toast.makeText(this, getResources().getString(R.string.vt_sd_not_enough), Toast.LENGTH_SHORT).show();
            	return;
            }else
            {
				(new Thread() {
					public void run() {
						...
						if (DBG) log("onVTTakePeerPhotoClick(): VTManager.getInstance().savePeerPhoto(), return "+ ret);
						if (ret){
	...
	{    	
    	if(DBG)log("debugVTUIInfo : output the Visibility info : ");
    	if(DBG)log(" - debugVTUIInfo : mVTPhoneNumberIcon - " + mVTPhoneNumberIcon.getVisibility());
    	if(DBG)log(" - debugVTUIInfo : mVTPhoneNumber - " + mVTPhoneNumber.getVisibility());
    	if(DBG)log(" - debugVTUIInfo : mVTElapsedTimeIcon - " + mVTElapsedTimeIcon.getVisibility());
    	if(DBG)log(" - debugVTUIInfo : mVTElapsedTime - " + mVTElapsedTime.getVisibility());
    	if(DBG)log(" - debugVTUIInfo : mVTTakePeerPhoto - " + mVTTakePeerPhoto.getVisibility());
    	if(DBG)log(" - debugVTUIInfo : mVTSwitchCamera - " + mVTSwitchCamera.getVisibility());
    	if(DBG)log(" - debugVTUIInfo : mVTHideMe - " + mVTHideMe.getVisibility());
    	if(DBG)log(" - debugVTUIInfo : mVTMute - " + mVTMute.getVisibility());
    	if(DBG)log(" - debugVTUIInfo : mVTSpeaker - " + mVTSpeaker.getVisibility());
    	if(DBG)log(" - debugVTUIInfo : mVTHangUp - " + mVTHangUp.getVisibility());
    	
    	if(DBG)log("debugVTUIInfo : output the enable info : ");
    	if(DBG)log(" - debugVTUIInfo : mVTTakePeerPhoto - " + mVTTakePeerPhoto.isEnabled());
    	if(DBG)log(" - debugVTUIInfo : mVTSwitchCamera - " + mVTSwitchCamera.isEnabled());
    	if(DBG)log(" - debugVTUIInfo : mVTHideMe - " + mVTHideMe.isEnabled());
    	if(DBG)log(" - debugVTUIInfo : mVTMute - " + mVTMute.isEnabled());
    	if(DBG)log(" - debugVTUIInfo : mVTSpeaker - " + mVTSpeaker.isEnabled());
    	if(DBG)log(" - debugVTUIInfo : mVTHangUp - " + mVTHangUp.isEnabled());
	}
