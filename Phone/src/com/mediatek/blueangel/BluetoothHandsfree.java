...
public class BluetoothHandsfree {
    ...
    private final MTKCallManager mMCM;	// For GEMINI case
    ...
    /* package */ synchronized void onBluetoothEnabled() {
        ...
        //    mIncomingSco = createScoSocket();
            /* SH : Comment */
            /* It will create a thread to listen incoming SCO connection */
        //    mIncomingSco.accept();
        //}
        /* MTK Removed : End */
    }
	...
	private static final int PHONE_VT_RING_INFO = 13;
        ...
        private final BroadcastReceiver mStateReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                log("[API] mStateReceiver.onReceive("+intent.getAction()+")");
        ...
        private synchronized void handlePreciseCallStateChange(boolean sendUpdate,
            ...
            case ACTIVE:
                log("Forground call active");
                call = 1;
                mAudioPossible = true;
                break;
            case DIALING:
                log("Forground call dialing");
                callsetup = 2;
                mAudioPossible = true;
                ...
                audioOn(); /* Follow the SPEC, start audio when dialing */
                break;
            case ALERTING:
                log("Forground call alerting");
                callsetup = 3;
        ...
        private void registerPhoneEvents(boolean register) {
            ...
            if(register) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                    /* +GEMINI SUPPORT */
                    /* Register for SIM1 */
                    mMCM.registerForPreciseCallStateChangedGemini(mStateChangeHandler, PRECISE_CALL_STATE_CHANGED, null, Phone.GEMINI_SIM_1);
        	          // SH : Merge Yufeng's modification
    	              mMCM.registerForIncomingRingGemini(mStateChangeHandler, PHONE_INCOMING_RING, null, Phone.GEMINI_SIM_1);
                    mMCM.registerForCallWaitingGemini(mStateChangeHandler, PHONE_CDMA_CALL_WAITING, null, Phone.GEMINI_SIM_1);
                    /* Register for SIM2 */
                    mMCM.registerForPreciseCallStateChangedGemini(mStateChangeHandler, PRECISE_CALL_STATE_CHANGED, null, Phone.GEMINI_SIM_2);
        	          // SH : Merge Yufeng's modification
    	              mMCM.registerForIncomingRingGemini(mStateChangeHandler, PHONE_INCOMING_RING, null, Phone.GEMINI_SIM_2);
                    mMCM.registerForCallWaitingGemini(mStateChangeHandler, PHONE_CDMA_CALL_WAITING, null, Phone.GEMINI_SIM_2);
		    if( FeatureOption.MTK_VT3G324M_SUPPORT == true )
		    {
		       	mMCM.registerForVtRingInfoGemini(mStateChangeHandler, PHONE_VT_RING_INFO, null, Phone.GEMINI_SIM_1);
		       	mMCM.registerForVtRingInfoGemini(mStateChangeHandler, PHONE_VT_RING_INFO, null, Phone.GEMINI_SIM_2);
		    }                    
                    /* -GEMINI SUPPORT */
                }else {
                    mCM.registerForPreciseCallStateChanged(mStateChangeHandler,
                            PRECISE_CALL_STATE_CHANGED, null);
                    mCM.registerForIncomingRing(mStateChangeHandler, 
    				PHONE_INCOMING_RING, null);
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                    /* +GEMINI SUPPORT */
                    mMCM.unregisterForPreciseCallStateChangedGemini(mStateChangeHandler, Phone.GEMINI_SIM_1);
                    mMCM.unregisterForIncomingRingGemini(mStateChangeHandler, Phone.GEMINI_SIM_1);
                    mMCM.unregisterForCallWaitingGemini(mStateChangeHandler, Phone.GEMINI_SIM_1);
                    mMCM.unregisterForPreciseCallStateChangedGemini(mStateChangeHandler, Phone.GEMINI_SIM_2);
                    mMCM.unregisterForIncomingRingGemini(mStateChangeHandler, Phone.GEMINI_SIM_2);
                    mMCM.unregisterForCallWaitingGemini(mStateChangeHandler, Phone.GEMINI_SIM_2);
	            if( FeatureOption.MTK_VT3G324M_SUPPORT == true )
        	    {
		        mMCM.unregisterForVtRingInfoGemini(mStateChangeHandler, Phone.GEMINI_SIM_1);
			mMCM.unregisterForVtRingInfoGemini(mStateChangeHandler, Phone.GEMINI_SIM_2);
		    }
                    /* -GEMINI SUPPORT */
                }else {
    ...
    private static final int CHECK_OUT_CALL_STARTED = 7;
            ...
            synchronized (BluetoothHandsfree.this) {
                ...
                case MESSAGE_CHECK_PENDING_SCO:
                    if (mPendingSco && isA2dpMultiProfile()) {
                        logWarn("Timeout suspending A2DP for SCO (mA2dpState = " +
    ...
    */
    ...
    /* package */ synchronized void audioOff() {
        if (VDBG) log("audioOff(): mPendingSco: "+mPendingSco+/*", mConnectedSco: "+
                mConnectedSco+", mOutgoingSco: "+mOutgoingSco+*/
    ...
    private void initializeHandsfreeAtParser() {
        ...
        parser.register('A', new AtCommandHandler() {
            ...
            public AtCommandResult handleBasicCommand(String args) {
                ...
                mBluetoothPhoneState.stopRing();
                if(FeatureOption.MTK_VT3G324M_SUPPORT == true &&
                    PhoneApp.getInstance().isVTRinging()) {
                    try{
                        PhoneApp.getInstance().touchAnswerVTCall();
                    }catch(Exception ex){
