...
public class SuggestionsProvider extends android.content.ContentProvider {
    ...
    private static final String LOG_TAG = "SuggestionsProvider";
    ...
    private class SuggestionsCursor implements CrossProcessCursor {
        ...
        private class Row {
            ...
            public String getWord() {
				...
				byte[] btarray = null;
				try{
					btarray = mText.getBytes(CHARSET_NAME);
				} catch (UnsupportedEncodingException ex) {
					Log.d(LOG_TAG, "UnsupportedEncodingException");
				}

				try {
					candidate = new String(btarray, mStartOffset, mEndOffset - mStartOffset, CHARSET_NAME);
				} catch (IndexOutOfBoundsException ex1) {
					Log.d(LOG_TAG, "IndexOutOfBoundsException");
...
//                  int termNumber   = offsets[j+1];
					...
					String candidate=null;
					byte[] btarray = null;
					Log.d(LOG_TAG,"computeRows. message="+message+". messge length: "+message.length() + "; startOffset="+startOffset+"; endOffset"+endOffset);
                    try{
						btarray = message.getBytes(CHARSET_NAME);
					} catch (UnsupportedEncodingException ex) {
						Log.d(LOG_TAG, "UnsupportedEncodingException");
					}

					try {
						candidate = new String(btarray, startOffset, endOffset - startOffset, CHARSET_NAME);
					} catch (IndexOutOfBoundsException ex1) {
						Log.d(LOG_TAG, "IndexOutOfBoundsException");
					} catch (NullPointerException ex2) {
						Log.d(LOG_TAG, "IndexOutOfBoundsException");
					...
					Log.d(LOG_TAG, "btarray=" + btarray + "; result ="
							+ candidate);
