...
public class WorkingMessage {
    ...
    public void setHasEmail(boolean hasEmail, boolean notify) {
        Log.v("Mms/compose", "WorkingMessage.setHasEmail(" + hasEmail + ", " + notify + ")");
    ...
    private void updateState(int state, boolean on, boolean notify) {
        Log.v("Mms/compose", "WorkingMessage.updateState(" + state + ", " + on + ", " + notify + ")");
        if (!sMmsEnabled) {
...
//        }
    ...
    private void sendMmsWorker(Conversation conv, Uri mmsUri, PduPersister persister,
        ...
        // Make sure we are still using the correct thread ID for our
        // recipient set.
        long threadId = conv.ensureThreadId(true /* scrubForMmsAddress */);
    ...
    private void asyncUpdateDraftMmsMessage(final Conversation conv) {
        ...
        new Thread(new Runnable() {
            public void run() {
                conv.ensureThreadId(true /* scrubForMmsAddress */);
    ...
    public void sendGemini(int simId) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Enter sendGemini(). SIM_ID = " + Integer.toString(simId, 10));
        }
    ...
    // 2.1
/*
    private void sendSmsWorkerGemini(Conversation conv, String msgText, int simId) {
        Log.d("Dual SIM MMS", "Enter sendSmsWorkerGemini(). SIM_ID = " + Integer.toString(simId, 10));
        mStatusListener.onPreMessageSent();
        // Make sure we are still using the correct thread ID for our
        // recipient set.
        long threadId = conv.ensureThreadId();
        ...
        MessageSender sender = new SmsMessageSender(mActivity, dests, msgText, threadId);
        try {
            sender.sendMessageGemini(threadId, simId);
...
*/
 ...
 /*   
    private void sendSmsWorkerGemini(Conversation conv, String msgText, int simId) {
        Log.d("MMSLog", "Enter sendSmsWorkerGemini(). SIM_ID = " + Integer.toString(simId, 10));
        mStatusListener.onPreMessageSent();
        // Make sure we are still using the correct thread ID for our
        // recipient set.
        long threadId = conv.ensureThreadId();
...
*/
    ...
    private void preSendSmsWorkerGemini(Conversation conv, String msgText, int simId) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "preSendSmsWorkerGemini. SIM ID = " + simId);
        }
        // If user tries to send the message, it's a signal the inputted text is what they wanted.
        UserHappinessSignals.userAcceptedImeText(mActivity);
    ...
    private void sendSmsWorkerGemini(String msgText, String semiSepRecipients, long threadId, int simId) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "sendSmsWorkerGemini. SIM ID = " + simId);
        }
        String[] dests = TextUtils.split(semiSepRecipients, ";");
        MessageSender sender = new SmsMessageSender(mActivity, dests, msgText, threadId, simId);
        try {
...
/*  // add for gemini 2.1
    private void sendMmsWorkerGemini(Conversation conv, Uri mmsUri, int simId, PduPersister persister,
                                   SlideshowModel slideshow, SendReq sendReq) {
            Log.d("MMSLog", "Enter sendMmsWorkerGemini(). SIM_ID = " + Integer.toString(simId, 10)+"  mmsUri="+mmsUri);
            // First make sure we don't have too many outstanding unsent message.
            Cursor cursor = null;
            try {
                ...
                if (cursor != null) {
                    ...
                    if (totalPendingSize >= maxMessageSize) {
                        Log.d("MMSLog", "sendMmsWorkerGemini(). total Pending Size >= max Message Size");
                        unDiscard();    // it wasn't successfully sent. Allow it to be saved as a draft.
                        mStatusListener.onMaxPendingMessagesReached();
                        return;
                    }
            ...
            if (mmsUri == null) {
                Log.d("MMSLog", "sendMmsWorkerGemini(). createDraftMmsMessage()");
                // Create a new MMS message if one hasn't been made yet.
                mmsUri = createDraftMmsMessage(persister, sendReq, slideshow);
            } else {
                Log.d("MMSLog", "sendMmsWorkerGemini(). updateDraftMmsMessage()");
                // Otherwise, sync the MMS message in progress to disk.
                updateDraftMmsMessage(mmsUri, persister, slideshow, sendReq);
            }
            Log.d("MMSLog", "sendMmsWorkerGemini(). mmsUri="+mmsUri);
...
*/
    ...
    private void sendMmsWorkerGemini(Conversation conv, Uri mmsUri, int simId, 
                                                ...
                                                SendReq sendReq) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Enter sendMmsWorkerGemini(). SIM_ID = " + Integer.toString(simId, 10) 
                + "  mmsUri=" + mmsUri);
        }
        ...
        if (mmsUri == null) {
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "create Draft Mms Message()");
            }
            // Create a new MMS message if one hasn't been made yet.
            mmsUri = createDraftMmsMessage(persister, sendReq, slideshow);
        } else {
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "update Draft Mms Message()");
            }
            // Otherwise, sync the MMS message in progress to disk.
            updateDraftMmsMessage(mmsUri, persister, slideshow, sendReq);
        }
