...
public class ContactList extends ArrayList<Contact> {
	...
	public static ContactList getByNumbers(Context mContext,
		...
		if (semiSepNumbers.contains(";")) {
			...
			Log.v("Mms/compose", "ContactList:getByNumber()--selection = \"" + selection + "\"");
			selection = DatabaseUtils.sqlEscapeString(selection);
			Cursor cursor = mContext.getContentResolver().query(
					PHONES_WITH_PRESENCE_URI, CALLER_ID_PROJECTION, selection,
					null, null);
			Log.v("Mms/compose", "ContactList:getByNumber()-1");
			if (cursor == null) {
				Log.w("Mms/compose", "ContactList.getByNumbers(" + semiSepNumbers
						+ ") returned NULL cursor! contact uri used "
						+ PHONES_WITH_PRESENCE_URI);
				return list;
			}
			Log.v("Mms/compose", "ContactList:getByNumber()-2");
			try {
			...
			Log.v("Mms/compose", "ContactList:getByNumber()-3");
