...
*/
    ...
    public boolean isEquivalent(Transaction transaction) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Transaction: isEquivalent mId=" + mId 
        ...
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Transaction: isEquivalent Id1=" + Id1 + "\t Id2=" + Id2);
        }
    ...
    protected byte[] sendPdu(long token, byte[] pdu, String mmscUrl) throws IOException {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Transaction: sendPdu");
        }
    ...
    private void ensureRouteToHost(String url, TransactionSettings settings) throws IOException {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Transaction: ensureRouteToHost");
        }
