...
import android.provider.Telephony.CbSms.Intents;
...
public class CBMessageReceiverService extends Service {
    ...
    private static final Uri MESSAGE_URI = Telephony.CbSms.CONTENT_URI;
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (intent != null) {
                ...
                if (Telephony.CbSms.Intents.CB_SMS_RECEIVED_ACTION.equals(action)
                        || Telephony.Sms.Intents.SMS_CB_RECEIVED_ACTION.equals(action)
                        || Telephony.Sms.Intents.SMS_EMERGENCY_CB_RECEIVED_ACTION.equals(action)) {
    ...
    private Uri insertMessage(Context context, CbSmsMessage[] msgs) {
        // Build the helper classes to parse the messages.
        CbSmsMessage sms = msgs[0];
        // emergency message. TODO maybe need Open
//        if (sms.getMessageClass() == CbSmsMessage.MessageClass.CLASS_0) {
...
//        }
    ...
    // TODO  Need replace with CB Message
    private ContentValues getCBContentValue(CbSmsMessage msg, String body) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (null != si) {
                values.put(Telephony.CbSms.SIM_ID, (int)si.mSimId);
            } else {
                Log.v("MMSLog", "getCBContentValue:SIMInfo is null for slot " + slotId);
            }
        }
        values.put(Telephony.CbSms.DATE, new Long(System.currentTimeMillis()));
        // Channel ID is getting from getMessageID
        values.put(Telephony.CbSms.CHANNEL_ID, msg.getMessageID());
        values.put(Telephony.CbSms.READ, Integer.valueOf(0));
        values.put(Telephony.CbSms.BODY, body);
        return values;  
    }
}
