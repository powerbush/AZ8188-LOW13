...
public class RetrieveTransaction extends Transaction implements Runnable {
    ...
    private boolean mCmccSupport = true;    // remove this member when offical define macro published
    ...
    private String getContentLocation(Context context, Uri uri)  throws MmsException {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "RetrieveTransaction: getContentLocation()");
        }
    ...
    public void process() {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "RetrieveTransaction: process()");
        }
    ...
    public void run() {
        try {
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "RetrieveTransaction: run()");
            }
            ...
            if (mCmccSupport){
                ...
                if (nInd.getExpiry() < System.currentTimeMillis()/1000L) {
                    mExpiry = true;
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "The message is expired!");
                    }
                    try {
                        ...
                        Log.e("MMSLog", Log.getStackTraceString(t));
                    }
                }
            }
            ...
            if (null == retrieveConf) {
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "RetrieveTransaction: run(): Invalid M-Retrieve.conf PDU!!!");
                }
            ...
            if (isDuplicateMessage(mContext, retrieveConf)) {
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "RetrieveTransaction: run, DuplicateMessage");
                }
                ...
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "RetrieveTransaction: run, Store M-Retrieve.conf into Inbox");
                }
    ...
    private void sendAcknowledgeInd(RetrieveConf rc) throws MmsException, IOException {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "RetrieveTransaction: sendAcknowledgeInd()");
        }
        ...
        if (tranId != null) {
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "RetrieveTransaction: sendAcknowledgeInd, Send ACK to mmsc");
            }
    ...
    */
    private void sendNotifyRespInd(int status) throws MmsException, IOException {
        ...
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "RetrieveTransaction: sendNotifyRespInd for expired.");
        }
    ...
    private static void updateContentLocation(Context context, Uri uri,
                                              ...
                                              boolean locked) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "RetrieveTransaction: updateContentLocation()");
        }
