...
public class MmsSystemEventReceiver extends BroadcastReceiver {
    ...
    private static void wakeUpServiceGemini(Context context, int simId) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "wakeUpServiceGemini: start transaction service ... simId="+simId);
        }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(Mms.Intents.CONTENT_CHANGED_ACTION)) {
            ...
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "Mms.Intents.CONTENT_CHANGED_ACTION: "+changed);
            }
            ...
            if (state.equals("CONNECTED")) {
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "TelephonyIntents.ACTION_ANY_DATA_CONNECTION_STATE_CHANGED, state=CONNECTED");
                }
                // add for gemini
                if(FeatureOption.MTK_GEMINI_SUPPORT == true){
                    ...
                    if (null == si) {
                        Log.v("MMSLog", "System event receiver: SIMInfo is null for slot " + intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, -1));
                        return;
                    }
            ...
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "Intent.ACTION_BOOT_COMPLETED");
            }
    ...
    public static void setPendingMessageFailed(final Context context) {
        Log.d("MMSLog", "setPendingMessageFailed");
        ...
        if (cursor != null) {
            try {
                ...
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "setPendingMessageFailed: Pending Message Size=" + count);
                }
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "setPendingMessageFailed: type=" + type + "; MsgId=" + id);
                    }
                ...
                // Ignore
                Log.e("MMSLog", "SQLiteDiskIOException caught while set pending message failed", e);
            } finally {
                cursor.close();
            }
        } else {
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "setPendingMessageFailed: no pending messages.");
            }
        }
    }

    public static void setNotificationIndUnstarted(final Context context) {
        Log.d("MMSLog", "setNotificationIndUnstarted");
        ...
        if (cursor != null) {
            try {
                int count = cursor.getCount();
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "setNotificationIndUnstarted: Message Size=" + count);
                }
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "setNotificationIndUnstarted: MsgId=" + id + "; status=" + status);
                    }
                ...
                // Ignore
                Log.e("MMSLog", "SQLiteDiskIOException caught while set notification ind unstart", e);
            } finally {
                cursor.close();
            }
        } else {
