...
//        }
        if (MESSAGE_SENT_ACTION.equals(intent.getAction())) {
            ...
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "Message Sent Result Code = "+mResultCode);
            }
        }
...
//        }
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "Sms handleMessage :" + msg);
            }
    ...
    private void handleServiceStateChanged(Intent intent) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Sms handleServiceStateChanged");
        }
        ...
        if (serviceState.getState() == ServiceState.STATE_IN_SERVICE) {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                if (null == si) {
                    Log.v("MMSLog", "handleServiceStateChanged:SIMInfo is null for slot " + slotId);
                    return;
                }
    ...
    private void handleSendMessage(Intent intent) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "handleSendMessage() simId="+intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, -1));
        }
        if (!mSending) {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
    ...
    public synchronized void sendFirstQueuedMessage() {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "sendFirstQueuedMessage()");
        }
    ...
    private void updateSizeForSentMessage(Intent intent){
        ...
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "update size for sent sms, size=" + messageSize);
        }
    ...
    private void handleSmsSent(Intent intent, int error) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "handleSmsSent(), errorcode=" + error);
        }
        //if (mResultCode == SmsManager.RESULT_ERROR_FDN_CHECK_FAILURE) {
        //    mToastHandler.sendEmptyMessage(FDN_CHECK_FAIL);
        //}
        Uri uri = intent.getData();
        mSending = false;
        ...
        if (mResultCode == Activity.RESULT_OK) {
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "handleSmsSent(), result is RESULT_OK");
            }
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            ...
            if (mResultCode == SmsManager.RESULT_ERROR_RADIO_OFF) {
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "handleSmsSent(), result is RESULT_ERROR_RADIO_OFF");
                }
            }else{
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "handleSmsSent(), result is RESULT_ERROR_NO_SERVICE");
                }
            }
            if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
    ...
    private void messageFailedToSend(Uri uri, int error) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "messageFailedToSend(),uri="+uri+"\terror="+error);
        }
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
    ...
    private void handleSmsReceived(Intent intent, int error) {
        ...
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "handleSmsReceived" + (tmpsms.isReplace() ? "(replace)" : "") 
                + " messageUri: " + messageUri 
     ...
     */
     private Uri insertMessage(Context context, Intent intent, int error) {
        ...
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Sms insertMessage");
        }
        SmsMessage[] msgs = Intents.getMessagesFromIntent(intent);
        SmsMessage sms = msgs[0];
...
/*    
    private Uri insertMessage(Context context, SmsMessage[] msgs, int error) {
        // Build the helper classes to parse the messages.
        Log.v("MMSLog", "Sms insertMessage");
        SmsMessage sms = msgs[0];
...
*/
    ...
    private Uri replaceMessage(Context context, SmsMessage[] msgs, int error) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Sms replaceMessage");
        }
        SmsMessage sms = msgs[0];
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (null == si) {
                Log.v("MMSLog", "SmsReceiverService:SIMInfo is null for slot " + sms.getMessageSimId());
                return null;
            }
            int simId = (int)si.mSimId;
    ...
    private Uri storeMessage(Context context, SmsMessage[] msgs, int error) {
        ...
        synchronized (MmsApp.getApplication()){
            if (((threadId == null) || (threadId == 0)) && (address != null)) {
                for (int i = 0; (i < 5) && ((threadId == null) || (threadId == 0)); ++i){
                    try{
                        ...
                        Log.e("MMSLog", "getOrCreateThreadId failed for this time:" + i);
                    }
                }
                //threadId = Threads.getOrCreateThreadId(context, address);
                values.put(Sms.THREAD_ID, threadId);
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "address=" + address + "\tthread id = " + threadId);
                }
            }
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "storeMessage. SIM ID = " + (int)si.mSimId);
                }
        ...
        String storeLocation = prefs.getString(MessagingPreferenceActivity.SMS_SAVE_LOCATION, "Phone");
        if (storeLocation.equals("Sim")) {
            ...
            for (int i = 0; i < pduCount; i++) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    ...
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "save sms on SIM. part:" + i 
                    ...
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "save sms on SIM. part:" + i + "; result:" + bSucceed + "; sc:" + sc);
                    }
                }
            }
        }
     ...
     */
     private void displayClassZeroMessage(Context context, Intent intent) {
        ...
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "Sms displayClassZeroMessage");
        }
...
/*    
    private void displayClassZeroMessage(Context context, SmsMessage sms) {
        // Using NEW_TASK here is necessary because we're calling
        // startActivity from outside an activity.
        Log.v("MMSLog", "Sms displayClassZeroMessage");
...
*/
    ...
    public synchronized void sendFirstQueuedMessageGemini(int simId) {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "sendFirstQueuedMessageGemini() simId=" + simId);
        }
        boolean success = true;
        // get all the queued messages from the database
        final Uri uri = Uri.parse("content://sms/queued");
        ...
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    ...
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "sendFirstQueuedMessage " + msgUri 
                            + ", address: " + address 
                            + ", threadId: " + threadId 
                            + ", body: " + msgText 
                            + ", simId: " + simId);
        ...
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "there is pending sms:" + (c == null?false:true));
        }
        if (success && c == null) {
