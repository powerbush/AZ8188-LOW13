...
public class NotificationTransaction extends Transaction implements Runnable {
    ...
    public void process() {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "NotificationTransaction: process()");
        }
    ...
    public void run() {
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "NotificationTransaction: run()");
        }
        ...
        try {
            ...
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "expiry time="+ExpiryDate.toLocaleString()
            ...
            if (optr.equals("OP01")) {
                ...
                if (cursor != null && cursor.getCount() == 1 && cursor.moveToFirst()) {
                    try{
                        ...
                        if (MmsApp.DEBUG) {
                            Log.d("MMSLog", "msg Size = " + msgSize);
                        }
                    }finally{
                        cursor.close();
                    }
                }
                String netWorkType = SystemProperties.get(TelephonyProperties.PROPERTY_CS_NETWORK_TYPE);
                ...
                if ((!bTDNetwork && MmsConfig.getReceiveMmsLimitFor2G() < msgSize/1024)
                    ...
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "Message size exceed limitation, rejected.");
                    }
            ...
            */
            ...
            if (!autoDownload || dataSuspended) {
                ...
                sendNotifyRespInd(status);
                if (!autoDownload && MmsApp.DEBUG){
                    Log.d("MMSLog", "Not autoDownload! so defer download");
                }
                if (dataSuspended && MmsApp.DEBUG){
                    Log.d("MMSLog", "data is suspended, so defer download");
                }
                return;
            }
            ...
            try {
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "NotificationTransaction: before getpdu");
                }
                retrieveConfData = getPdu(mContentLocation);
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "NotificationTransaction: after getpdu");
                }
            } catch (IOException e) {
            ...
            if (retrieveConfData != null) {
                ...
                if ((pdu == null) || (pdu.getMessageType() != MESSAGE_TYPE_RETRIEVE_CONF)) {
                    ...
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "NotificationTransaction: PDU Saved, Uri="+uri);
                        Log.d("MMSLog", "NotificationTransaction: Delete Notify Ind, Uri="+mUri);
                    }
                    // add for gemini
                    if (FeatureOption.MTK_GEMINI_SUPPORT) {
                        ...
                        if (MmsApp.DEBUG) {
                            Log.d("MMSLog", "save retrieved mms, simId="+mSimId);
                        }
                    }
                ...
                // keep this else here for further 
                if (MmsApp.DEBUG) {
                    Log.d("MMSLog", "retrieveConfData is null");
                }
                mTransactionState.setState(FAILED);
                status = STATUS_UNRECOGNIZED;                
            }
            ...
            switch (status) {
                case STATUS_RETRIEVED:
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "NotificationTransaction: status=STATUS_RETRIEVED");
                    }
                    mTransactionState.setState(SUCCESS);
                    break;
                case STATUS_DEFERRED:
                    if (MmsApp.DEBUG) {
                        Log.d("MMSLog", "NotificationTransaction: status=STATUS_DEFERRED");
                    }
            ...
            if (mTransactionState.getState() != SUCCESS) {
                mTransactionState.setState(FAILED);
                if (MmsApp.DEBUG) {
                    Log.e("MMSLog", "NotificationTransaction failed.");
                }
            }
            notifyObservers();
        }
    ...
    private void sendNotifyRespInd(int status) throws MmsException, IOException {
        ...
        if (MmsApp.DEBUG) {
            Log.d("MMSLog", "NotificationTransaction: sendNotifyRespInd()");
        }
