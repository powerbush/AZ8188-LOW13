...
public class PushReceiver extends BroadcastReceiver {
    ...
    private class ReceivePushTask extends AsyncTask<Intent,Void,Void> {
        ...
        protected Void doInBackground(Intent... intents) {
            ...
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "do In Background, slotId="+intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, 0));
            }
            ...
            try {
                switch (type) {
                    case MESSAGE_TYPE_DELIVERY_IND:
                        if (MmsApp.DEBUG) {
                            Log.d("MMSLog", "type=MESSAGE_TYPE_DELIVERY_IND");
                        }
                    case MESSAGE_TYPE_READ_ORIG_IND: {
                        if (MmsApp.DEBUG) {
                            Log.d("MMSLog", "type=MESSAGE_TYPE_READ_ORIG_IND");
                        }
                    ...
                    case MESSAGE_TYPE_NOTIFICATION_IND: {
                        if (MmsApp.DEBUG) {
                            Log.d("MMSLog", "type=MESSAGE_TYPE_NOTIFICATION_IND");
                        }
                        ...
                        if (!isDuplicateNotification(mContext, nInd)) {
                            ...
                            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                                ...
                                if (null == si) {
                                    Log.v("MMSLog", "PushReceiver:SIMInfo is null for slot " + intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, -1));
                                    break;
                                }
                                values.put(Mms.SIM_ID, si.mSimId);
                                SqliteWrapper.update(mContext, cr, uri, values, null, null);
                                if (MmsApp.DEBUG) {
                                    Log.d("MMSLog", "save notification slotId=" 
                                ...
                                if (pendingCs != null) {
                                    try {
                                        if ((pendingCs.getCount() == 1) && pendingCs.moveToFirst()) {
                                            ...
                                            Log.d("MMSLog", "can not find message to set pending sim id, msgId="+msgId);
                                        }
                                    }finally {
                                        pendingCs.close();
                                    }
    ...
    public void onReceive(Context context, Intent intent) {
        if (intent.getAction().equals(WAP_PUSH_RECEIVED_ACTION)
            ...
            // raise priority
            if (MmsApp.DEBUG) {
                Log.d("MMSLog", "raise priority");
            }
            raisePriority(context, true);

            if (MmsApp.DEBUG) {
                HashMap<String, String> wspHeaders = (HashMap<String, String>)intent.getSerializableExtra("wspHeaders");
                Log.d("MMSLog", "Map="+wspHeaders);
            }
