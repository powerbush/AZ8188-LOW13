    ...
    MessageItem(Context context, String type, Cursor cursor,
        ...
        if ("sms".equals(type)) {
            ...
            if (mSimMsg) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT){
                    localNumber = MessageUtils.getLocalNumberGemini(mSimId);
                } else {
                    localNumber = MessageUtils.getLocalNumber();
                }
                if (null != localNumber && localNumber.length() > Contact.STATIC_KEY_BUFFER_MAXIMUM_LENGTH 
                        && null != mContact && mContact.length() > Contact.STATIC_KEY_BUFFER_MAXIMUM_LENGTH) {
                    String subLocalNumber = localNumber.substring(localNumber.length() - Contact.STATIC_KEY_BUFFER_MAXIMUM_LENGTH);
                    String subContact = mContact.substring(mContact.length() - Contact.STATIC_KEY_BUFFER_MAXIMUM_LENGTH);
                    if (subLocalNumber != null && subLocalNumber.equals(subContact)) {
                        mContact = context.getString(R.string.messagelist_sender_self);
                    }
                }
