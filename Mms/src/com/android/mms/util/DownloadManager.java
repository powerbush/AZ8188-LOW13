...
public class DownloadManager {
    ...
    private final BroadcastReceiver mRoamingStateListener =
        ...
        public void onReceive(Context context, Intent intent) {
            if (TelephonyIntents.ACTION_SERVICE_STATE_CHANGED.equals(intent.getAction())) {
                ...
                if (MmsApp.DEBUG) {
                    Log.v("MMSLog", "roaming ------> " + isRoaming + ", slotId=" + intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, -1));
                }
                synchronized (sInstance) {
                    ...
                    if (FeatureOption.MTK_GEMINI_SUPPORT) {
                        ...
                        if (null == si) {
                            Log.v("MMSLog", "Download manager:SIMInfo is null for slot " + intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, -1));
                            return;
                        }
                        int simId = (int)si.mSimId;
                        mAutoDownload = getAutoDownloadState(mPreferences, isRoaming, simId);
                        if (MmsApp.DEBUG) {
                            Log.v("MMSLog", "mAutoDownload ------> " + mAutoDownload + ", simId=" + simId);
                        }
                    } else {
                    mAutoDownload = getAutoDownloadState(mPreferences, isRoaming);
                        if (MmsApp.DEBUG) {
                            Log.v("MMSLog", "mAutoDownload ------> " + mAutoDownload);
                        }
                    }
                }
            }
    ...
    private boolean getAutoDownloadState(SharedPreferences prefs, boolean roaming, int simId) {
        ...
        if (MmsApp.DEBUG) {
            Log.v("MMSLog", "sim " + simId + " auto download without roaming -> " + autoDownload);
        }
        ...
        if (autoDownload) {
            ...
            if (MmsApp.DEBUG) {
                Log.v("MMSLog", "sim " + simId + " auto download during roaming -> " + alwaysAuto);
            }
