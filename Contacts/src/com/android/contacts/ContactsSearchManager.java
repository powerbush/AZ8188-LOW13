...
public class ContactsSearchManager {
    ...
    //MTK
    public static final String ORIGINAL_DATA_EXTRA_KEY = "originalData";
    ...
    private static Intent buildIntent(Activity context, String initialQuery) {
        ...
        Uri origData = originalIntent.getData();
        String origType = originalIntent.getType();
        ...
        intent.putExtra("filter_sim_contact", originalIntent.getBooleanExtra("filter_sim_contact", false));
        intent.putExtra("broadcast_result", originalIntent.getBooleanExtra("broadcast_result", false));
        if (origData != null) {
            intent.putExtra(ORIGINAL_DATA_EXTRA_KEY, origData);
            Log.i("ContactsSearchManager", origData.toString());
        }
        if (origType != null) {
            intent.putExtra(ORIGINAL_TYPE_EXTRA_KEY, origType);
            Log.i("ContactsSearchManager", origType);
        }
        return intent;
    }
}
