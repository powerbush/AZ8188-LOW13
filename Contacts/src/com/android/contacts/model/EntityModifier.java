...
public class EntityModifier {
    ...
    public static boolean isEmpty(ValuesDelta values, DataKind kind) {
        //MTK for group
        if (GroupMembership.CONTENT_ITEM_TYPE.equals(kind.mimeType)) {
            final Integer id = values.getAsInteger(GroupMembership.GROUP_ROW_ID);
            if (id != null) {
                return false;
            } else {
                return true;
    ...
    public static void parseExtras(Context context, ContactsSource source, EntityDelta state,
        ...
        {
        	DataKind kind = null;
        	final String number = extras.getString(Insert.PHONE);
        	Log.i(TAG,"number IS " + number);
        	Log.i(TAG,"PhoneNumberUtils.isUriNumber(number) IS " + PhoneNumberUtils.isUriNumber(number));
        	if (!PhoneNumberUtils.isUriNumber(number)) {        		
            // Phone
                kind = source.getKindForMimetype(Phone.CONTENT_ITEM_TYPE);
            ...
            ContactsUtils.setMoreButtonStatus(false);
        		...
        		// Sip call
        		kind = source.getKindForMimetype(SipAddress.CONTENT_ITEM_TYPE);
        		ContactsUtils.setMoreButtonStatus(true);
        		Log.i(TAG,"************sip call **************");
        	}            
        ...
        if (hasOrg && EntityModifier.canInsert(state, kindOrg)) {
            ...
            if (ContactsUtils.isGraphic(company)) {
                if (child != null) child.put(Organization.COMPANY, company);
            }

            final String title = extras.getString(Insert.JOB_TITLE);
            if (ContactsUtils.isGraphic(title)) {
            	if (child != null) child.put(Organization.TITLE, title);
            }
        }
