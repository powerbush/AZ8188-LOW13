...
public class ImportVCardActivity extends Activity {
    ...
    public static final String LAUNCHCONTACT =
    ...
    private boolean mSDCardRemoved = false;
    ...
    private class ImportTypeSelectedListener implements
        ...
        public void onClick(DialogInterface dialog, int which) {
            if (which == DialogInterface.BUTTON_POSITIVE) {
              ...
              {
                runOnUIThread(new DialogDisplayer(R.id.dialog_sdcard_not_found));//SD Card was remounted.
              }
    ...
    private class VCardScanThread extends Thread implements OnCancelListener, OnClickListener {
        ...
        public void setSDState(boolean bRemoved){
            mSDMoved=bRemoved;
        }
        public void cancel() {
            mCanceled = true;
        ...
        public void run() {
            ...
            if(mSDMoved)
            {
                runOnUIThread(new DialogDisplayer(R.id.dialog_sdcard_not_found));
            }else if (mGotIOException) {
    ...
    private void startVCardSelectAndImport() {
        ...
        if (getResources().getBoolean(R.bool.config_import_all_vcard_from_sdcard_automatically)) {
            importMultipleVCardFromSDCard(mAllVCardFileList);
        }// commented by mtk80909 for ALPS00124949 
        /*else if (size == 1) {
    ...
    protected void onCreate(Bundle bundle) {
        ...
        if ((intent.getFlags() & Intent.FLAG_ACTIVITY_LAUNCHED_FROM_HISTORY) != 0) {
            Intent intentContact = new Intent(LAUNCHCONTACT);
    ...
    private BroadcastReceiver mScanListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {        
            if (intent.getAction().equals(Intent.ACTION_MEDIA_UNMOUNTED)
                ...
                Log.i(LOG_TAG,"Intent.ACTION_MEDIA_UNMOUNTED");
                if (mVCardScanThread != null) {            
                  mVCardScanThread.setSDState(true);
                  mSDCardRemoved=true;
                  mVCardScanThread.cancel();
                }
            }else if(intent.getAction().equals(Intent.ACTION_MEDIA_MOUNTED)){
               mSDCardRemoved = false;
               Log.i(LOG_TAG,"Intent.ACTION_MEDIA_MOUNTED");
            }
        }
    };
}
